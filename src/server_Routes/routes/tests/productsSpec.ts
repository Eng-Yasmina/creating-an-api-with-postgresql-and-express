...
describe('Product Route', () => {
    ...
    it('shuold create a new product', async () => {
        const result = await productsRoutes.post('/api/products').set('Content-type', 'application/json').send({ token: `${utoken}` }).send({
    ...
    it('shuold get all products', async () => {
        const result = await productsRoutes.get('/api/products').set('Content-type', 'application/json')
        expect(result.status).toBe(200);
    ...
    it('shuold get a specific product', async () => {
        const result = await productsRoutes.get(`/api/products/1`).set('Content-type', 'application/json')
        expect(result.status).toBe(200);
